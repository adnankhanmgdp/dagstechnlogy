...
exports.fetchServices = async (req, res) => {
    ...
    try {
        const service = await Service.find().sort('serviceId');
        res.json({
...
exports.createOrder = async (req, res) => {
    try {
        ...
        //tracking all the order amount for all orders without tax,deliverycharge etc.
        ...
        taxAmount = ((allAmount + deliveryFee * 2) * misc.tax) / 100
        ...
        let total = (allAmount + deliveryFee * 2 + misc.platformFee + taxAmount - discountedPrice).toFixed(2);
...
exports.verifyPayment = async (req, res) => {
    ...
    try {
        ...
        if (payment.status === "captured") {
            ...
            order.orderDate = new Date(Date.now() + (5.5 * 60 * 60 * 1000))
            await order.save();
...
exports.fetchAllOrders = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ phone });
        const orders = await Order.find({ userId: phone }).sort({
            orderDate: -1
        })
